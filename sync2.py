...
class CommunicationApp:
    ...
    def create_widgets(self):
            ...
            tk.Button(left_frame, text="Browse", command=lambda: self.browse_file(self.input_file1_entry)).grid(row=1,column=2,padx=5,pady=5)
            ...
            tk.Button(left_frame, text="Browse", command=lambda: self.browse_file(self.output_file1_entry)).grid(row=2,column=2,padx=5,pady=5)
            ...
            self.port1_transmit_clock_combobox = ttk.Combobox(left_frame, textvariable=self.port1_transmit_clock_var, state="readonly")
            ...
            self.port1_receive_clock_combobox = ttk.Combobox(left_frame, textvariable=self.port1_receive_clock_var, state="readonly")
            ...
            tk.Button(center_frame, text="Configure Ports", command=self.configure_ports).grid(row=0, column=0, padx=5, pady=5)
            ...
            tk.Button(right_frame, text="Browse", command=lambda: self.browse_file(self.input_file2_entry)).grid(row=1, column=2, padx=5, pady=5)
            ...
            tk.Button(right_frame, text="Browse", command=lambda: self.browse_file(self.output_file2_entry)).grid(row=2, column=2, padx=5, pady=5)
            ...
            self.port2_transmit_clock_combobox = ttk.Combobox(right_frame, textvariable=self.port2_transmit_clock_var, state="readonly")
            ...
            self.port2_receive_clock_combobox = ttk.Combobox(right_frame, textvariable=self.port2_receive_clock_var, state="readonly")
    ...
    def configure_ports(self):
        ...
        try:
            ...
            configure_port(self.port1, port1_data_rate, port1_transmit_clock, port1_receive_clock, port1_protocol,port1_arayuz, port1_async_data_rate, port1_data_bit,port1_stop_bit,port1_parity)
            self.port1.enable_receiver()
            self.port2 = open_port(port2_name)
            configure_port(self.port2, port2_data_rate, port2_transmit_clock, port2_receive_clock, port2_protocol,port2_arayuz,port2_async_data_rate,port2_data_bit,port2_stop_bit,port2_parity)
    ...
    def send_data(self, port, port_name, input_file, running_check, label, bits_counter):
        try:
            ...
            while running_check():
                ...
                if port_name == self.port1_entry.get():
                    self.sent_bits_port1 += len(buf) * 8
                else:
                    self.sent_bits_port2 += len(buf) * 8
            ...
            with open(output_file, 'w') as f:
                while running_check():
                    ...
                    if port.name == self.port1_entry.get():
                        self.received_bits_port1 += len(buf) * 8
                    else:
                        self.received_bits_port2 += len(buf) * 8
